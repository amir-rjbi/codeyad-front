<template>
  <div class="mt-3">
        ...
        <tbody>
          ...
          <template v-else-if="courses && courses.length > 0">
            <tr v-for="course in courses" :key="course.id" @click="setSelected(course)">
              <td>{{ course.id }}</td>
              <td>
                <base-button class="mx-auto" :render-button-tag="false" :to="`/account/courses/show/${course.id}`"
                  color-white>
                  {{ course.courseTitle }}
                </base-button>
              </td>
              ...
              <td  class="flex lg:inherit sm:relative justify-center border-none">
                <div class="w-16 h-12 flex absolute overflow-y-visible items-center gap-2 cursor-pointer"
                  ...
                  <IconsArrowLeft class="w-full justify-self-start pointer-events-none transition-all"
                    :style="{ rotate: showMenu ? '90deg' : '-90deg' }" />
    ...
    <div class="w-full flex items-center justify-center mt-4" v-if="coursesResult">
...
const showOptions = (e: any) => {
  var elements = document.getElementsByClassName('account-menu');
  for (var i = 0; i < elements.length; i++) {
    if (elements[i].id == e.target.getAttribute('data-selector')) {
      continue;
    }
