<template>
  <div>
    ...
    <base-button color="green" @click="b_createSpec=true">افزودن ویژگی جدید</base-button>
          ...
          <tr v-for="spec in specifications" :key="spec.id">
            ...
            <td width="5%">
              <div class="flex items-center space-x-2 space-x-reverse">
                <base-button outline color-white class="mx-auto" title="ویرایش ویژگی" @click="selected = spec, b_editSpec = true">
                  <IconsEdit width="16" height="16" color="green" />
                </base-button>
                <base-button outline color-white class="mx-auto" @click="deleteSpecification(spec.id)" title="حذف ویژگی">
    ...
    <base-modal title="افزودن ویژگی جدید" v-model="b_createSpec">
      <account-courses-specifications-add :course-id="courseId" @specCreated="specificationCreated"/>
    </base-modal>

    <base-modal title="ویرایش ویژگی" v-model="b_editSpec">
      <account-courses-specifications-edit :specification="selected" @specEdited="specificationCreated"/>
    </base-modal>
...
const specificationCreated = async ()=>{
  b_createSpec.value = false;
  b_editSpec.value = false;
  await getData;
}

const deleteSpecification = async (specId: number) => {
  loading.value = true;
  const fetchResult = await RemoveCourseSpecification(specId);
  if (fetchResult.isSuccess) {
