<template>
  <div v-if="loading" class="flex flex-col gap-4">
    <BaseSkeletonLoaidng width="100%" parent-class="w-[400px]" />
  </div>
  <Form v-else v-slot="{ errors }" :validation-schema="schema" @submit="editCourse" class="flex flex-wrap">
    <div class="w-1/2 p-2 sm:w-full sm:px-0">
      <BaseInput placeholder="عنوان    " out-line name="title" label="عنوان دوره:" v-model="data.title" />
    </div>
    <div class="w-1/2 p-2 sm:w-full sm:px-0">
      <BaseInput placeholder="  عنوان انگلیسی    " out-line name="slug" label="عنوان انگلیسی (slug):"
        v-model="data.slug" />
    </div>
    <div class="w-full p-2 sm:w-full sm:px-0">
      <BaseInput placeholder="قیمت دوره  " type="number" out-line name="price" label=" قیمت دوره : (0 = رایگان)"
        v-model="data.price" />
    </div>
    <div class="w-full p-2 sm:px-0">
      <BaseHtmlEditor label="توضیحات: " name="description" v-model="data.description" id="question_text2" />
    </div>
    <div class="w-full p-2 sm:px-0">
      <BaseTextArea label="  توضیحات کوتاه: " name="shortDescription" v-model="data.shortDescription"
        id="shortDescription" placeholder="توضیح کوتاهی از دوره را وارد کنید" />
    </div>
    <div class="w-1/2 p-2 sm:w-full sm:px-0">
      <BaseSelectBox required v-model="data.categoryId" label="دسته بندی اصلی" out-line name="categoryId"
        @change="categorySelected($event, null)">
        <option v-if="categories && categories.length > 0" v-for="c in categories" :key="c.id" :value="c.id">{{ c.title }}
        </option>
      </BaseSelectBox>
    </div>
    <div class="w-1/2 p-2 sm:w-full sm:px-0">
      <BaseSelectBox required v-model="data.subCategoryId" label="دسته بندی دوم" out-line name="subCategoryId">
        <option v-if="subCategories && subCategories.length > 0" v-for="c in subCategories" :key="c.id" :value="c.id">
          {{ c.title }}</option>
      </BaseSelectBox>
    </div>
    <div class="w-full sm:px-0">
      <BaseSelectBox required v-model="data.courseLevel" label="سطح دوره  " out-line name="courseLevel">
    ...
    <div class="w-full p-2 sm:w-full sm:px-0">
      <BaseInput placeholder="پیشنیاز ها را با - از هم جدا کنید, مثال - c#-js    " out-line name="requirements"
        label=" پیش نیاز های دوره:" v-model="data.requirements" />
    </div>
    ...
    <div class="w-1/2 p-2 sm:w-full sm:px-0">
      <BaseInput placeholder="فرمت صحیح: 00:06:27" out-line name="demoDuration" label="مدت زمان ویدئو" dir="ltr"
...
onMounted(async () => {
  ...
  if (fetchResult) {
    ...
    if (categoriesResult.isSuccess) {
      ...
      categorySelected(null, categories.value.find(c => c.id == Number(data.categoryId)));

    }

    data.title = fetchResult.data?.courseTitle!;
    data.categoryId = fetchResult.data!.categoryId!.toString();


    data.subCategoryId = fetchResult.data!.subCategoryId!.toString();
    data.price = fetchResult.data!.price!.toString();
    ...
    data.demoDuration = fetchResult.data?.duration!;
  }
  loading.value = false;
})

const categorySelected = (event: any, category: CourseCategory | null = null) => {
  let selectedCategory: CourseCategory | null = null;
